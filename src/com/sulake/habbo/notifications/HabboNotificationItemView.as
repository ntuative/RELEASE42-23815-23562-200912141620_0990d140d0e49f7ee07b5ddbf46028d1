package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements ICoreUpdateReceiver
   {
      ...
      public static const const_666:int = 70;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar)
      {
         ...
         if(_loc7_ != null)
         {
            var_1222 = 0 - _loc7_.rectangle.bottom;
         }
         else
         {
            var_1222 = 15;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _window.height = 0;
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + var_1222;
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(_state)
         {
            ...
            case const_366:
               var_820 += param1;
               _loc2_ = Number(var_820) / Number(var_387["time_fade_in"]);
               if(var_820 > int(var_387["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_752:
               var_991 += param1;
               if(var_991 > int(var_387["time_display"]) && !var_1223)
               {
                  startFadeOut();
               }
               break;
            case const_753:
               var_819 += param1;
               _loc3_ = 1 - Number(var_819) / Number(var_387["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_819 > int(var_387["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_190.orientation == "TOP" ? int(const_338) : 0;
         var _loc4_:int = var_190.orientation == "BOTTOM" ? int(_loc2_.height - const_338) : int(_loc2_.height);
         var _loc5_:int = var_190.orientation == "LEFT" ? int(const_338) : 0;
         var _loc6_:int = var_190.orientation == "RIGHT" ? int(_loc2_.width - const_338) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_398;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_398;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_551;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_551;
            }
         }
         else if(false)
         {
