package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_1250:TextFieldManager;
      ...
      private var var_770:TextFieldManager;
      ...
      private function clearErrors() : void
      {
         ...
         this.var_1250.clearErrors();
         this.var_769.clearErrors();
         this.var_770.clearErrors();
      ...
      {
         Logger.log("[RoomSettingsCtrl.onDeleteButtonClick] " + var_397);
         _navigator.registerParameter("navigator.roomsettings.deleteroom.confirm.message","room_name",var_79.name);
         var _loc2_:ConfirmDialogView = new ConfirmDialogView(IFrameWindow(_navigator.getXmlWindow("ros_room_delete_confirm")),this,onConfirmRoomDelete,[var_397]);
         _loc2_.show();
      }
      ...
      {
         return IWindowContainer(_navigator.windowManager.createWindow(param1,"",HabboWindowType.const_54,HabboWindowStyle.const_39,HabboWindowParam.const_64,new Rectangle(0,0,100,20)));
      }
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_2043.visible = var_479;
         var_2042.visible = !var_479;
         if(var_479)
         {
         ...
         else
         {
            ...
            var_1252.visible = _inRoom;
            _navigator.thumbRenderer.refreshThumbnail(IWindowContainer(var_1252.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_1010.height = Util.getLowestPoint(this.var_1010) + 4;
         }
         var_1468.text = !!var_479 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1465.name,var_1252.name,var_1010.name,var_1008.name,var_1254.name,var_1467.name],0,0);
      ...
      private function populateForm() : void
      {
         ...
         var_838.setText(_loc1_.name);
         var_1250.setText(_loc1_.description);
         var_1009.setText("");
         var_1251.setText("");
         ...
         {
            case RoomSettingsData.const_154:
         ...
         setTag(var_769,_loc1_.tags[0]);
         setTag(var_770,_loc1_.tags[1]);
         if(var_1253)
         {
            if(_loc1_.allowPets)
            {
      ...
      {
         this.var_397 = param1;
         _navigator.send(new GetRoomSettingsMessageComposer(var_397));
         if(var_1007 != null)
         {
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         _loc2_.name = var_838.getText();
         _loc2_.description = var_1250.getText();
         var _loc3_:ISelectorWindow = _window.findChildByName("doormode") as ISelectorWindow;
         var _loc4_:IWindow = _loc3_.getSelected();
         switch(_loc4_.name)
         {
            case "doormode_doorbell":
               _loc2_.doorMode = RoomSettingsFlatInfo.const_154;
               break;
         ...
         {
            _loc8_ = var_1009.getText();
            _loc9_ = var_1251.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_1009.clearErrors();
               this.var_1251.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowPets = var_1253.isSelected;
         _loc2_.allowFoodConsume = var_1255.isSelected;
         ...
         addTag(var_769,_loc2_.tags);
         addTag(var_770,_loc2_.tags);
         this.clearErrors();
         this.var_1011 = _loc2_.roomId;
      ...
      private function onRemoveAllFlatCtrlsClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _navigator.send(new RemoveAllRightsMessageComposer(this.var_397));
      }
      
      public function onRoomSettingsSaved(param1:int) : void
      {
         if(param1 != this.var_397 || var_1011 < 1)
         {
            return;
         }
         close();
      ...
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         ...
         var_1011 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1325)
         {
            this.var_838.displayError("${navigator.roomsettings.roomnameismandatory}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1206)
         {
            this.var_838.displayError("${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1197)
         {
            this.var_1250.displayError("${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1344)
         {
            setTagError(this.var_769,param3,"${navigator.roomsettings.unacceptablewords}");
            setTagError(this.var_770,param3,"${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1145)
         {
            setTagError(this.var_769,param3,"${navigator.roomsettings.nonuserchoosabletag}");
            setTagError(this.var_770,param3,"${navigator.roomsettings.nonuserchoosabletag}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1297)
         {
            this.var_1009.displayError("${navigator.roomsettings.passwordismandatory}");
         }
         else
         {
            this.var_838.displayError("Update failed: error " + param2);
         }
      }
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_521,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_962,onDoorModePasswordUnselect);
         ...
         Util.setProc(_window,"remove_flat_ctrl",method_10);
         refreshCategories();
         this.var_838 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("room_name")),60);
         this.var_1250 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("description")),255);
         this.var_769 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag1")),30);
         this.var_770 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag2")),30);
         this.var_1009 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1251 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         this.var_1253 = ICheckBoxWindow(_window.findChildByName("allow_pets_checkbox"));
         this.var_1255 = ICheckBoxWindow(_window.findChildByName("allow_foodconsume_checkbox"));
         var_1465 = IWindowContainer(_window.findChildByName("header_container"));
         var_1252 = IWindowContainer(_window.findChildByName("thumbnail_container"));
         var_1010 = IWindowContainer(_window.findChildByName("basic_settings_container"));
         var_1008 = IWindowContainer(_window.findChildByName("advanced_settings_container"));
         var_1467 = IWindowContainer(_window.findChildByName("switch_view_container"));
         var_1254 = IWindowContainer(_window.findChildByName("footer_container"));
         ...
         var_2042 = ITextWindow(_window.findChildByName("basic_caption"));
         var_2043 = ITextWindow(_window.findChildByName("advanced_caption"));
         var_1468 = ITextWindow(_window.findChildByName("switch_view_text"));
         var_1468.procedure = onSwitchView;
         var_1007.visible = false;
         if(!_inRoom)
         {
