package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1594:int = 3;
      
      public const const_1499:int = 2;
      ...
      private function onConfirmDialogEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            ...
            case "confirm_buy_button":
               var_70.buyMarketPlaceOffer(var_503);
               if(var_124)
               {
                  var_124.dispose();
               }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            if(parser.offerId > 0)
            {
               newItem = var_209.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
               var_209.remove(parser.requestedOfferId);
               newItem.offerId = parser.offerId;
               newItem.price = parser.newPrice;
               var_209.add(parser.offerId,newItem);
            }
            else
            {
               var_209.remove(parser.requestedOfferId);
         ...
         {
            var_209.remove(parser.offerId);
            var_503 = -1;
            _visualization.listUpdatedNotify();
         }
         else if(parser.result == 3)
         {
            updateItem = var_209.getValue(var_503) as MarketPlaceOfferData;
            var_209.remove(parser.requestedOfferId);
            updateItem.offerId = parser.offerId;
            updateItem.price = parser.newPrice;
            var_209.add(parser.offerId,updateItem);
            showConfirmation(const_1499);
            _visualization.listUpdatedNotify();
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_592.add(_loc4_.offerId,_loc5_);
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_592.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_592.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_443)
            {
               var_592.remove(_loc2_);
            }
         }
      ...
      private function showConfirmation(param1:int) : void
      {
         ...
         switch(param1)
         {
            case const_1430:
               ...
               var_124 = _windowManager.buildFromXML(_loc2_) as IWindowContainer;
               break;
            case const_1499:
         ...
         if(var_124)
         {
            var_124.procedure = onConfirmDialogEvent;
            var_124.visible = true;
            var_124.center();
            _loc3_ = var_209.getValue(var_503) as MarketPlaceOfferData;
            if(!_loc3_)
            {
               return;
            }
            _loc4_ = var_124.findChildByName("item_name") as ITextWindow;
            if(_loc4_ != null)
            {
               _loc4_.text = "${roomItem.name." + _loc3_.furniId + "}";
            }
            _loc5_ = var_124.findChildByName("item_desc") as ITextWindow;
            if(_loc5_ != null)
            {
               _loc5_.text = "${roomItem.desc." + _loc3_.furniId + "}";
            }
            _loc6_ = var_124.findChildByName("item_price") as ITextWindow;
            if(_loc6_ != null)
            {
